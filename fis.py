...
from FuzzySystem import fuzzyvariable
...
class FuzzyInferenceSystem:
    ...
    def eval(self, inputs, verbose=None):
        ...
        for rule in self.rules:
             output = rule.eval(inputs, and_op=self.and_op, or_op=self.or_op, verbose=verbose)
             rule_outputs.append((rule, output))

        if verbose:
             print("\nFuzzy System Rules with Outputs:")
             for rule, output in rule_outputs:
                  print(f"Rule: {rule}")
